...
function readData() {
  ...
  // Migrazione: aggiungi nuovi campi settings se mancanti
...
app.get('/api/settings', (req, res) => {
  const data = readData();
  const { site_email, site_nome, disponibilita, risposta_tempi, site_telefono, site_indirizzo, site_citta, site_sito } = data.settings;
  res.json({ site_email, site_nome, disponibilita, risposta_tempi, site_telefono, site_indirizzo, site_citta, site_sito });
});
...
app.post('/api/prenota', (req, res) => {
  ...
  // ── Invio email lato server (non blocca la risposta) ──
  const s = readData().settings;
  ...
  // Conferma al cliente (se template configurato)
...
// ─── Admin API: Upload foto articolo ──────────────────────
app.post('/api/admin/catalogo/:id/foto', adminAuth, upload.single('foto'), (req, res) => {
  ...
  const idx  = data.catalogo.findIndex(x => x.id === req.params.id);
  if (idx === -1) {
    // Articolo non trovato: elimina il file caricato
...
// ─── Admin API: Elimina foto articolo ─────────────────────
...
app.listen(PORT, () => {
  ...
  // Leggi la password corrente dal data.json (se è stata cambiata dall'admin non la conosciamo in chiaro)
  // Mostriamo la password di default se il hash corrisponde, altrimenti avvisiamo
  ...
  console.log('');
  console.log(`  ${c.bold}${c.white}🖨  Print3D Studio${c.reset}`);
